...
include_once($projectroot."admin/includes/templates/adminimage.php");
...
if($action==="rename")
{
  renamecaption($filename,$caption);
  $messagetitle="Renamed caption";
...
elseif($action==="addimage")
{
  ...
  else
  {
    ...
    {
      $success= uploadfile($_FILES,getproperty("Image Upload Path"),"filename",$filename);
    }
    if($success)
    {
      ...
      if($thumbnail)
      {
        ...
        $thsuccess=uploadfile($_FILES,getproperty("Image Upload Path"),"thumbnail",$newthumbname);
...
elseif($action==="replaceimage")
{
  ...
  else
  {
    ...
    if($success)
    {
      $success= replacefile($_FILES,getproperty("Image Upload Path"),"newfilename",$filename);
    }
    if($success)
    {
...
elseif($action==="cat")
{
  ...
  {
    removeimagecategories($filename,$selectedcats);
    $messagetitle="Removed categories from image";
    $displayeditform=true;
  }
  elseif(isset($_POST['addcat']))
  {
    addimagecategories($filename,$selectedcats);
    $messagetitle="Added new categories for image";
    $displayeditform=true;
  }
}
elseif($action==="addthumb")
{
  ...
  if($thumbnail)
  {
    ...
    $imagename=substr($filename,0,strrpos($filename,"."));
    $newthumbname.=$imagename.'_thn'.$extension;
    $success=uploadfile($_FILES,getproperty("Image Upload Path"),"thumbnail",$newthumbname);
    $thumbnail=$newthumbname;
...
elseif($action==="replacethumb")
{
  ...
  else
  {
    ...
    //else 
    $success=checkextension($thumbnailfilename,$thumbnail);
    if($success)
    {
      $success= replacefile($_FILES,getproperty("Image Upload Path"),"thumbnail",$thumbnailfilename);
    }
    if($success)
    {
...
{
  renamesource($filename,$source);
  $messagetitle="Renamed image source";
  $displayeditform=true;
}
elseif($action==="sourcelink")
{
  renamesourcelink($filename,$sourcelink);
  $messagetitle="Changed image source link";
  $displayeditform=true;
}
elseif($action==="copyright")
{
  setimagecopyright($filename,$copyright);
  $messagetitle="Changed Copyright Holder";
  $displayeditform=true;
}
elseif($action==="permission")
{
  setimagepermission($filename,$permission);
  $messagetitle="Changed Permission";
  $displayeditform=true;
}
...
{
  $form=new DeleteImageConfirmForm($filename,true);
}
elseif($action==="deletefile")
{
  ...
  {
    deletefile(getproperty("Image Upload Path"),$filename);
...
elseif($action==="executethumbnaildelete")
{
	if(isset($_POST['delete']))
  	{
    	$messagetitle="Deleting thumbnail for image <i>".$filename."</i></i>";
    	if(hasthumbnail($filename))
    	{
      		$thumbnail=getthumbnail($filename);
      		$success=deletefile(getproperty("Image Upload Path"),$thumbnail);
      		...
      		{
        		$message="Failed to delete file <i>".$thumbnail."</i>";
      		}
    	}
    	else
    	{
